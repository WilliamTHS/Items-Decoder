...
int main()
{
	...
	int size = file.tellg();
	if(size == -1) {
		cout << "[GTNOOBS] Failed to read the file, did you put the file together with this tool?, if not make sure you move the file into the folder with the tool" << std::endl;
		return 0;
	}
	...
	{
		cout << "[GTNOOBS] Starting to decode the file you requested" << endl;

	}
	else {
		cout << "[GTNOOBS] An error occurred while decoding your file, please try again later" << endl;
#ifndef __linux__
	_getch();
#endif
		exit(0);
	...
	for (int i=0;i<itemCount;i++) {
		...
		} // items dat version 12
		if(itemsdatVersion >= 12) {
			memPos += 13;
		} // items dat version 13
        if (itemsdatVersion >= 13) {
            memPos += 4;
        } // items dat version 14
        if(itemsdatVersion >= 14) {
            memPos +=4;
        } 
		if (i != itemID)
			cout << "[GTNOOBS] Items cannot be parsed" << endl;
		j["itemID"] = itemID;
	...
	cout << "[GTNOOBS] Your file has been successfully decoded" << endl;
#ifndef __linux__
	_getch();
#endif
	return 0;
